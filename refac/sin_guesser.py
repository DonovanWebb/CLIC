...
def clines(c_line_dict, l1_l2, s1, s2, r):
    l1, l2 = tuple(myround(l1_l2, r)%360)
...
def count_dict(ldict):
    ...
    print('mean group ', np.mean(sizes))
    print('std group ', np.std(sizes))
    '''
    return groups


def choose_rand_group(groups):
    count = 0
    for g in groups:
        if len(g) >= 6 and np.random.randint(100) >= 95:
            return g
        elif count > 2000:
            print("No large group found!")
            exit()
        else: count += 1


def main(projs=700, rounding=3):
    com_lines = {}
    all_sins = get_dataset('../../dsets/exp_plan/angles/7_5_angles.star')
    ...
    # for s1 in range(10):
        # for s2 in range(80, 90):
            ...
            com_lines = clines(com_lines, l1_l2, s1, s2, rounding)
            l1_l2_2 = (l1_l2 + 180)%360  # to get mirror
            com_lines = clines(com_lines, l1_l2_2, s1, s2, rounding)
    groups = count_dict(com_lines)
    return groups
